import { useEffect, useMemo, useState } from "react";
import {
    Box, Paper, IconButton, Switch, Typography, Tab, Tabs,
    Collapse, TextField, Button, Dialog, DialogContent,
    DialogActions, useTheme,
} from "@mui/material";
import AccessTimeIcon from "@mui/icons-material/AccessTime";
import BarChartIcon from "@mui/icons-material/BarChart";
import AddIcon from "@mui/icons-material/Add";
import DescriptionIcon from "@mui/icons-material/Description";
import CommonChartSwitcher from "./CommonChart";
import { useParams } from "react-router-dom";
import RemoveIcon from '@mui/icons-material/Remove';
import CommonDialog from "./CommonDialog";
import { drawerData } from "../../data/drawerData";
import CollapsibleSection from "./CollapsibleSection";
import { useQuery } from "@tanstack/react-query";
import { mainData } from "../../api/authApi";
import { appendInitialData } from "./reusableFunctions";
import { useSelector } from "react-redux";
import "../../loading.css"

const commonFlexStyle = {
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    gap: "10px"
};

const oddsBoxStyle = (bgcolor) => ({
    bgcolor: bgcolor,
    borderRadius: 1,
    fontWeight: 500,
    minWidth: "40px",
    height: "fit-content",
    textAlign: "center",
    cursor: "pointer"
});
const suspendedBoxStyle = {
    background: "linear-gradient(to right, #e6f7ff 50%, #fff0f0 50%)",
    borderRadius: 1,
    textAlign: "center",
    p: 1,
    border: "1px solid #ddd",
    fontWeight: 600,
    fontSize: "14px",
    height: "fit-content",
    cursor: "pointer"
};

const emptyOddsBoxStyle = {
    display: "flex",
    minWidth: "50px",
    textAlign: "center",
    borderRadius: "4px",
    bgcolor: "#f0f0f0",
    alignItems: "center",
    justifyContent: "center"
};

const oddsColumnStyle = (bgcolor) => ({
    display: "flex",
    flexDirection: "column",
    bgcolor,
    minWidth: "50px",
    textAlign: "center",
    borderRadius: "4px",
    py: 0.7,
    cursor: "pointer",
    maxHeight: "50px"
});

const CommonMatch = () => {
    const { segment } = useParams();
    const theme = useTheme();
    const [clickBetting, setClickBetting] = useState(false);
    const [acceptOdds, setAcceptOdds] = useState(false);
    const [collapse, setCollapse] = useState(true);
    const [tab, setTab] = useState(0);
    const [values, setValues] = useState([20, 50, 100]);
    const [readOnly, setReadOnly] = useState(true);
    const [open, setOpen] = useState(false);
    const [selectedTab, setSelectedTab] = useState("betSlip");
    const [stake, setStake] = useState(0);
    const [profit, setProfit] = useState(0);
    const [dialogOpen, setDialogOpen] = useState(false);
    const [dialogData, setDialogData] = useState();
    const [bestSliptData, setBestSliptData] = useState({
        isOpen: false,
        label: '',
        odds: null,
        bgcolor: '',
    });
    const [markedRawData, setMarketRewData] = useState(null);
    const [marketsByType, setMarketsByType] = useState({});
    const { data: fetchedMatchData } = useQuery({
        queryKey: ["matchData", segment],
        queryFn: () => mainData(segment),
    });
    const rawData = useSelector((state) => state.getrawMarketData);

    const selectionIds = useMemo(() => {
        const ids = ["Bookmaker", "Match Odds"].flatMap(category =>
            (marketsByType?.[category] ?? []).flatMap(item =>
                (item.r ?? []).map(runner => runner.selectionId)
            )
        );
        return [...new Set(ids)];
    }, [marketsByType]);

    const matchData = fetchedMatchData?.data || {
        event: { mnm: '', st: new Date().toISOString() },
        markets: []
    };
    const tabLabels = ["All", ...Object.keys(marketsByType)];
    const inningsList = ["1st INNS", "2nd INNS"];
    const dialogSportData = {};
    drawerData?.sports?.forEach((sport) => {
        const sportName = sport?.sport;
        if (!sportName || !Array.isArray(sport?.leagues)) return;
        sport.leagues.forEach((league) => {
            const hasInfo = Array.isArray(league?.info) && league.info.length > 0;
            const subtitles = hasInfo
                ? league.info.map((infoItem) => infoItem?.title).filter(Boolean)
                : [];
            if (!dialogSportData[sportName]) {
                dialogSportData[sportName] = [];
            }
            dialogSportData[sportName].push({
                title: league.title?.trim() || '',
                subtitle: subtitles,
            });
        });
    });

    const presetAmounts = [500, 1000, 5000, "10K", "50K", "100K", "500K", "5M"];
    const parseAmount = (val) => {
        if (typeof val === "number") return val;
        return parseInt(val.replace("K", "000").replace("M", "000000"));
    };
    const handleDislogData = (data) => {
        setDialogData(data);
        setDialogOpen(true);
    };

    const calculateProfit = (currentStake, currentOdds) => {
        if (currentOdds) {
            return ((parseFloat(currentOdds)) * currentStake).toFixed(2);
        }
        return 0;
    };

    const handlePresetClick = (amount) => {
        const value = parseAmount(amount);
        const newStake = stake + value;
        setStake(newStake);
    };

    const handleStakeChange = (e) => {
        const value = parseFloat(e.target.value) || 0;
        setStake(value);
    };

    const adjustOdds = (value) => {
        const newOdds = Math.max(1, parseFloat(bestSliptData.odds) + value);
        setBestSliptData(prev => ({ ...prev, odds: newOdds.toFixed(2) }));
    };

    const incrementStake = () => {
        const newStake = stake + 10;
        setStake(newStake);
    };

    const decrementStake = () => {
        const newStake = Math.max(0, stake - 10);
        setStake(newStake);
    };

    const clearAll = () => {
        setBestSliptData((prev) => ({ ...prev, isOpen: false }));
        setStake(0);
        setProfit(0);
    };

    const handleAcceptToggle = () => {
        setAcceptOdds(!acceptOdds);
        setOpen(true);
    };

    const handleConfirm = () => {
        setOpen(true);
        setAcceptOdds(true);
        setOpen(false);
    };

    const handleDecline = () => {
        setAcceptOdds(false);
        setOpen(false);
    };

    const handleChange = (index, newValue) => {
        const updated = [...values];
        updated[index] = newValue;
        setValues(updated);
    };

    const handleToggle = () => {
        setCollapse((prev) => !prev);
    };

    const matchName = matchData?.event?.mnm || '';

    let teamA = '';
    let teamB = '';
    let shortA = '';
    let shortB = '';

    if (matchName.includes(' v ')) {
        const teams = matchName.split(' v ');
        teamA = teams[0]?.trim() ?? '';
        teamB = teams[1]?.trim() ?? '';
        shortA = teamA.slice(0, 2).toUpperCase();
        shortB = teamB.slice(0, 2).toUpperCase();
    } else {
        teamA = matchName.trim();
    }


    const lineChartData = {
        "1st INNS": [
            {
                name: "Bangladesh",
                data: [0, 2, 4, 5, 6, 6.5, 7, 8, 8.2, 8.5, 9, 9.5, 10, 9.8, 9.5, 9.2, 8.5, 8.0, 7.5, 7],
            },
            {
                name: "Sri Lanka",
                data: [0, 1, 2, 3, 4, 4.2, 4.5, 5, 6, 7, 7.5, 8, 8.3, 8.5, 8.7, 8.9, 9.0, 9.1, 9.2, 9.3],
            },
        ],
        "2nd INNS": [
            {
                name: "Bangladesh",
                data: [0, 1, 2, 2.5, 3.5, 4, 4.8, 5.2, 5.7, 6, 6.2, 6.8, 7.2, 7.5, 7.8, 8, 8.2, 8.4, 8.5, 9],
            },
            {
                name: "Sri Lanka",
                data: [0, 1, 1.5, 2, 2.5, 3, 3.4, 3.8, 4.2, 4.5, 5, 5.3, 5.8, 6.2, 6.5, 6.9, 7.1, 7.5, 7.8, 8],
            },
        ],
    };

    const barChartData = {
        "1st INNS": [
            {
                name: "Bangladesh",
                data: [4, 5, 6, 5.5, 4.5, 5.7, 6.3, 6, 7, 5, 6, 7, 8, 6, 5, 4, 4.2, 4.8, 3.9, 2],
            },
            {
                name: "Sri Lanka",
                data: [3.5, 4, 4.8, 4.2, 3.9, 4.1, 4.6, 5, 5.2, 5.5, 6, 6.2, 6.5, 6.8, 6.9, 7, 7.1, 7.3, 7.4, 7.5],
            },
        ],
        "2nd INNS": [
            {
                name: "Bangladesh",
                data: [2, 3, 4, 4.5, 5, 5.2, 5.8, 6, 6.5, 6.8, 7, 7.2, 7.5, 7.7, 7.9, 8.1, 8.2, 8.3, 8.4, 8.5],
            },
            {
                name: "Sri Lanka",
                data: [1, 2, 2.8, 3.5, 4, 4.5, 5, 5.2, 5.4, 5.8, 6, 6.3, 6.6, 6.8, 7, 7.3, 7.5, 7.7, 7.9, 8],
            },
        ],
    };

    useEffect(() => {
        if (!fetchedMatchData?.data?.markets?.length) {
            setMarketsByType({});
            return;
        }
        const matchesData = [...new Set(fetchedMatchData.data.markets.map(item => item.t))];
        const newState = {};
        matchesData.forEach(type => {
            newState[type] = fetchedMatchData.data.markets.filter(market => market.t === type);
        });
        setMarketsByType(newState);
    }, [fetchedMatchData]);

    useEffect(() => {
        const fetchAndAppendData = async () => {
            if (rawData && selectionIds.length > 0) {
                const getrawData = await appendInitialData(rawData, selectionIds);
                setMarketRewData(getrawData);
            }
        };

        fetchAndAppendData();
    }, [rawData, selectionIds]);

    useEffect(() => {
        setProfit(calculateProfit(stake, bestSliptData.odds));
    }, [stake, bestSliptData.odds]);

    const filteredMarkets = useMemo(() =>
        tab === 0
            ? Object.keys(marketsByType).reduce((acc, type) => [...acc, ...marketsByType[type]], [])
            : marketsByType[tabLabels[tab]] || []
        , [tab, tabLabels, marketsByType]);
    if (filteredMarkets == [] || markedRawData == null) {
        return (
            <Box sx={{ width: "100vw", height: "100vh", background: "#373737ff", }}>
                <Box sx={{ transform: " translate(40%, 150%)" }}>
                    <div className="loader"></div>
                </Box>
            </Box>

        )
    }
    return (
        <Box sx={{ position: "relative", mb: 4, p: 2 }}>
            <CommonDialog open={dialogOpen} data={dialogData} title={"Rules"} handleClose={() => setDialogOpen(false)} />
            <Dialog
                open={open}
                onClose={handleDecline}
                scroll="body"
                PaperProps={{
                    sx: {
                        borderRadius: 2, p: 2, minWidth: 300, textAlign: "center", position: "absolute",
                        top: { lg: "10%", md: "10%", sm: "15%", xs: "15%" }, left: "50%",
                        transform: "translate(-50%, -50%) !important",
                    }
                }}
            >
                <DialogContent>
                    <Typography sx={{ fontSize: "16px", fontWeight: 500 }}>
                        Please confirm that you're allowing exchange system to place bets
                        on any odds at Betfair accepted rate
                    </Typography>
                </DialogContent>
                <DialogActions sx={{ justifyContent: "center", gap: 2 }}>
                    <Button onClick={handleConfirm} variant="contained" sx={{ bgcolor: "green", px: 3 }}>
                        Accept
                    </Button>
                    <Button onClick={handleDecline} variant="contained" sx={{ bgcolor: "red", px: 3 }}>
                        Decline
                    </Button>
                </DialogActions>
            </Dialog>
            <Box sx={{ display: "flex", width: "100%", justifyContent: "space-between", flexWrap: "wrap" }}>
                <Box
                    sx={{
                        width: { lg: "59%", md: "59%", sm: "100%", xs: "100%" },
                        borderRadius: "5px",
                    }}
                >
                    <Paper
                        elevation={3}
                        sx={{
                            ...commonFlexStyle,
                            px: 1.5,
                            mb: 2, borderRadius: 2, minHeight: 60,
                        }}
                    >
                        <Box sx={commonFlexStyle}>
                            <AccessTimeIcon fontSize="small" />
                            <Typography variant="body1" fontWeight="medium" sx={{
                                display: { xs: "flex" }, flexDirection: { lg: "row", md: "row", sm: "row", xs: "column" },
                                fontSize: { lg: "15px", md: "15px", sm: "15px", xs: "10px" },
                            }}>
                                {new Date(matchData?.event?.st).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                            </Typography>
                        </Box>
                        <Typography fontWeight="bold">{matchName}</Typography>
                        <Box sx={commonFlexStyle}>
                            <IconButton size="small" sx={{ border: "1px solid black", borderRadius: "10px" }} onClick={handleToggle}>
                                {collapse ? <BarChartIcon /> : <BarChartIcon />}
                            </IconButton>
                            <IconButton size="small" sx={{ border: "1px solid black", borderRadius: "10px" }} onClick={() => handleDislogData(drawerData)}>
                                <AddIcon />
                            </IconButton>
                        </Box>
                    </Paper>
                    <Paper>
                        <Box
                            sx={{
                                ...commonFlexStyle,
                                mb: 2, px: 3, py: 1
                            }}
                        >
                            {shortA && (
                                <Box
                                    sx={{
                                        backgroundColor: "#f5f5f5de", width: 40, height: 40, borderRadius: "50%",
                                        ...commonFlexStyle,
                                        fontWeight: "bold", p: 2, justifyContent: "center"
                                    }}
                                >
                                    {shortA}
                                </Box>
                            )}
                            {teamB ? (
                                <Box sx={{ ...commonFlexStyle, gap: 3 }}>
                                    <>
                                        <Typography fontWeight="bold">{teamA.toUpperCase()}</Typography>
                                        <img src="https://img.freepik.com/premium-vector/versus-battle-illustration-logo-design-template-versus-vector-icon-vs-letters-sports_564974-202.jpg" alt="vs" style={{ height: 40, width: 40 }} />
                                        {<Typography fontWeight="bold">{teamB.toUpperCase()}</Typography>}
                                    </>
                                </Box>
                            ) : (
                                <Typography fontWeight="bold" sx={{ margin: "auto" }}>{teamA.toUpperCase()}</Typography>
                            )}
                            {shortB && (
                                <Box
                                    sx={{
                                        ...commonFlexStyle,
                                        backgroundColor: "#f5f5f5de", width: 40, height: 40, borderRadius: "50%",
                                        fontWeight: "bold", p: 2, justifyContent: "center"
                                    }}
                                >
                                    {shortB}
                                </Box>
                            )}
                        </Box>
                        <Collapse in={collapse} timeout="auto" unmountOnExit>
                            {matchData ? (
                                <Box>
                                    <CommonChartSwitcher
                                        lineChartData={lineChartData}
                                        barChartData={barChartData}
                                        inningsList={inningsList}
                                    />
                                </Box>
                            ) : (
                                <Box
                                    sx={{
                                        height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center',
                                        fontSize: '1.2rem', fontWeight: 500, bgcolor: "#000"
                                    }}
                                >
                                    No data available
                                </Box>
                            )}
                        </Collapse>
                        <Box sx={{ background: "#f5f5f5", borderRadius: 2 }}></Box>
                    </Paper>
                    <Box sx={{ display: "flex", gap: 2 }}>
                        <Box sx={{ flex: 1 }}>
                            <Tabs
                                value={tab}
                                onChange={(e, v) => setTab(v)}
                                sx={{ mt: 2 }}
                                TabIndicatorProps={{ style: { display: 'none' } }}
                            >
                                {tabLabels.map((label, idx) => (
                                    <Tab
                                        key={idx}
                                        label={label}
                                        sx={{
                                            border: "1px solid #ffcc00", fontWeight: 600,
                                            bgcolor: tab === idx ? "#ffcc00" : "",
                                            py: 0, "&.Mui-selected": { borderBottom: "none" },
                                        }}
                                    />
                                ))}
                            </Tabs>
                            {filteredMarkets.length > 0 ? (
                                <Box>
                                    {Object.keys(marketsByType).map((marketType) => (
                                        (tab === 0 || tabLabels[tab] === marketType) && marketsByType[marketType] && (
                                            <CollapsibleSection
                                                key={marketType}
                                                sectionKey={marketType}
                                                title={marketType}
                                                sx={{ bgcolor: theme.palette.background.default, borderRadius: 1, p: 2, mb: 2 }}
                                            >
                                                <>
                                                    {marketType === "Fancy" && (
                                                        <Box>
                                                            {marketsByType[marketType].map((fancyMarket, idx) => (
                                                                <Box
                                                                    key={fancyMarket.i}
                                                                    display="flex"
                                                                    justifyContent="space-between"
                                                                    alignItems="center"
                                                                    py={1}
                                                                    sx={{ borderBottom: "1px solid #ddd", "&:last-child": { borderBottom: "none" } }}
                                                                >
                                                                    <Box>
                                                                        <Typography fontSize="14px">{fancyMarket.n}</Typography>
                                                                    </Box>
                                                                    <Box sx={{ ...commonFlexStyle, gap: 1 }}>
                                                                        {fancyMarket.ms.name === "open" ? (
                                                                            [
                                                                                { odds: fancyMarket.do.toString().slice(0, 3), stake: fancyMarket.fi.toString().slice(0, 3), label: "No", bgcolor: "#ffcdd2" },
                                                                                { odds: fancyMarket.mn.toString().slice(0, 3), stake: fancyMarket.i.toString().slice(0, 3), label: "Yes", bgcolor: "#bbdefb" }
                                                                            ].map((entry, i) => (
                                                                                <Box
                                                                                    key={i}
                                                                                    sx={oddsBoxStyle(entry.bgcolor)}
                                                                                    onClick={() => setBestSliptData({
                                                                                        isOpen: true,
                                                                                        label: `${fancyMarket.n} ${entry.label}`,
                                                                                        odds: entry.odds,
                                                                                        bgcolor: entry.bgcolor,
                                                                                    })}
                                                                                >
                                                                                    <Typography variant="subtitle2" sx={{ height: "13px" }}>{entry.odds}</Typography>
                                                                                    <Typography variant="caption">{entry.stake}</Typography>
                                                                                </Box>
                                                                            ))
                                                                        ) : (
                                                                            <Box sx={suspendedBoxStyle}>
                                                                                {fancyMarket.ms.name.toUpperCase()}
                                                                            </Box>
                                                                        )}
                                                                    </Box>
                                                                </Box>
                                                            ))}
                                                        </Box>
                                                    )}
                                                    {marketType !== "Fancy" && marketsByType[marketType].map((market, idx) => (
                                                        <div key={idx}>
                                                            {market.r.map((runner, runnerIdx) => {
                                                                const matchedEntry = markedRawData?.find(entry => runner.selectionId === entry.selectionId);
                                                                return (
                                                                    <Box
                                                                        key={runnerIdx}
                                                                        sx={{
                                                                            ...commonFlexStyle,
                                                                            pb: 1,
                                                                            borderBottom: "1px solid #ddd", paddingTop: "10px", "&:last-child": { borderBottom: "none" },
                                                                        }}
                                                                    >
                                                                        <Box>
                                                                            <Typography fontWeight={500}>{runner.runnerName}</Typography>
                                                                            <Typography color="green">0</Typography>
                                                                        </Box>
                                                                        {market.ms?.name === "SUSPENDED" ? (
                                                                            <Box sx={suspendedBoxStyle}>
                                                                                SUSPENDED
                                                                            </Box>
                                                                        ) : (
                                                                            matchedEntry ? (
                                                                                <Box sx={{ ...commonFlexStyle, gap: 1 }}>
                                                                                    {matchedEntry.availableToBack?.map((data, idx) => (
                                                                                        <Box
                                                                                            key={`back-${idx}`}
                                                                                            sx={oddsColumnStyle("#ffcdd2")}
                                                                                            onClick={() =>
                                                                                                setBestSliptData({ isOpen: true, label: runner.runnerName, odds: data.price, bgcolor: "#ffcdd2" })
                                                                                            }
                                                                                        >
                                                                                            <Typography variant="subtitle2">{data.price}</Typography>
                                                                                            <Typography variant="caption">{data.size}</Typography>
                                                                                        </Box>
                                                                                    ))}
                                                                                    {matchedEntry.availableToLay?.map((data, idx) => (
                                                                                        <Box
                                                                                            key={`lay-${idx}`}
                                                                                            sx={oddsColumnStyle("#bbdefb")}
                                                                                            onClick={() =>
                                                                                                setBestSliptData({ isOpen: true, label: runner.runnerName, odds: data.price, bgcolor: "#bbdefb" })
                                                                                            }
                                                                                        >
                                                                                            <Typography variant="subtitle2">{data.price}</Typography>
                                                                                            <Typography variant="caption">{data.size}</Typography>
                                                                                        </Box>
                                                                                    ))}
                                                                                </Box>
                                                                            ) : (
                                                                                <Box sx={{ ...commonFlexStyle, gap: 1 }}>
                                                                                    {[...Array(6)].map((_, item) => (
                                                                                        <Box
                                                                                            key={item}
                                                                                            sx={emptyOddsBoxStyle}
                                                                                        >
                                                                                            <Typography variant="subtitle2">-</Typography>
                                                                                        </Box>
                                                                                    ))}
                                                                                </Box>
                                                                            )
                                                                        )}
                                                                    </Box>
                                                                );
                                                            })}
                                                        </div>
                                                    ))}
                                                </>
                                            </CollapsibleSection>
                                        )
                                    ))}
                                </Box>
                            ) : (
                                <Typography>No markets available</Typography>
                            )}
                        </Box>
                    </Box>
                </Box>
                <Box sx={{ width: { lg: "39%", md: "39%", sm: "100%", xs: "100%" } }}>
                    <Paper sx={{ p: 1, mb: 1 }}>
                        <Box display="flex" justifyContent="space-between" alignItems="center">
                            <Box sx={commonFlexStyle}>
                                <Switch checked={clickBetting} onChange={() => setClickBetting(!clickBetting)} />
                                <Typography>1 Click Betting</Typography>
                            </Box>
                            <Box sx={commonFlexStyle}>
                                <Switch checked={acceptOdds} onClick={handleAcceptToggle} />
                                <Typography>Accept any ODDS</Typography>
                            </Box>
                        </Box>
                        {clickBetting && (
                            <>
                                <Box sx={{ ...commonFlexStyle, flexWrap: "wrap", justifyContent: "center" }}>
                                    {values.map((val, index) => (
                                        <TextField
                                            key={index}
                                            size="small" type="number" value={val}
                                            onChange={(e) => handleChange(index, e.target.value)}
                                            inputProps={{ readOnly }} sx={{ py: 1, m: 1.5, flex: 1 }}
                                        />
                                    ))}
                                </Box>
                                <Button
                                    variant="contained" sx={{ display: "flex", margin: "auto", bgcolor: "#ffc107", color: "black" }}
                                    onClick={() => setReadOnly((prev) => !prev)}
                                >
                                    Edit
                                </Button>
                            </>
                        )}
                    </Paper>
                    <Paper sx={{ borderRadius: 2, overflow: "hidden", bgcolor: "#b2ebf2" }}>
                        <Box display="flex">
                            <Box
                                onClick={() => setSelectedTab("betSlip")}
                                sx={{
                                    flex: 1, py: 1, textAlign: "center",
                                    bgcolor: selectedTab === "betSlip" ? "#ffc107" : "#535558",
                                    color: selectedTab === "betSlip" ? "#000" : "#fff",
                                    fontWeight: 600, cursor: "pointer",
                                }}
                            >
                                <AddIcon fontSize="small" sx={{ verticalAlign: "middle", mr: 0.5 }} />
                                Bet Slip
                            </Box>
                            <Box
                                onClick={() => setBestSliptData((prev) => ({ ...prev, isOpen: false }))}
                                sx={{
                                    flex: 1, py: 1, textAlign: "center",
                                    bgcolor: selectedTab === "myBets" ? "#ffc107" : "#535558",
                                    color: selectedTab === "myBets" ? "#000" : "#fff",
                                    fontWeight: 600, cursor: "pointer",
                                }}
                            >
                                <DescriptionIcon fontSize="small" sx={{ verticalAlign: "middle", mr: 0.5 }} />
                                My Bets
                            </Box>
                        </Box>
                        {bestSliptData.isOpen ? (
                            <Box sx={{ p: 2, bgcolor: bestSliptData.bgcolor, textAlign: "center" }}>
                                <Typography variant="subtitle2" color="text.secondary">{matchName}</Typography>
                                <Box sx={{ ...commonFlexStyle, mt: 1 }}>
                                    <Typography>{bestSliptData.label}</Typography>
                                    <Box>
                                        <Typography fontSize="14px" fontWeight={600} color="green">Profit</Typography>
                                        <Typography fontSize="14px" fontWeight={600} color="green">₹ {profit}</Typography>
                                    </Box>
                                </Box>
                                <Box sx={{ ...commonFlexStyle, gap: 1, borderRadius: 1, width: "50%", border: "1px solid #000" }}>
                                    <Button size="small" sx={{ minWidth: 24, bgcolor: "#ddd", color: "black", borderRight: "1px solid #000" }} onClick={() => adjustOdds(-0.01)}>
                                        <RemoveIcon />
                                    </Button>
                                    <Typography fontWeight={600}>{bestSliptData.odds}</Typography>
                                    <Button size="small" sx={{ minWidth: 24, px: 1, bgcolor: "#ddd", color: "black", borderLeft: "1px solid #000" }} onClick={() => adjustOdds(0.01)}>
                                        <AddIcon />
                                    </Button>
                                </Box>
                                <Box sx={{ ...commonFlexStyle, mt: 2, borderRadius: 1, border: "1px solid #000" }}>
                                    <Button size="small" sx={{ minWidth: 24, px: 1, bgcolor: "#ddd", color: "black", borderRight: "1px solid #000" }} onClick={decrementStake}><RemoveIcon /></Button>
                                    <TextField
                                        placeholder="Stake" variant="standard" value={stake}
                                        onChange={handleStakeChange} fullWidth
                                        InputProps={{ disableUnderline: true }}
                                        sx={{ mx: 1, input: { textAlign: "center" } }}
                                    />
                                    <Button size="small" sx={{ minWidth: 24, px: 1, bgcolor: "#ddd", color: "black", borderLeft: "1px solid #000" }} onClick={incrementStake}><AddIcon /></Button>
                                </Box>
                                <Box sx={{ ...commonFlexStyle, mt: 2, flexWrap: "wrap", justifyContent: "center", gap: 1 }}>
                                    {presetAmounts.map((val, idx) => (
                                        <Button
                                            key={idx} variant="contained" size="small"
                                            sx={{ bgcolor: "#e0e0e0", color: "#000", flex: 1 }}
                                            onClick={() => handlePresetClick(val)}
                                        >
                                            {val}
                                        </Button>
                                    ))}
                                    <Button
                                        variant="outlined" size="small" sx={{ gridColumn: "span 3", borderColor: "#000", color: "#000", width: "100%" }}
                                        onClick={() => { setStake(0) }}
                                    >
                                        Clear
                                    </Button>
                                </Box>
                                <Box sx={{ ...commonFlexStyle, mt: 2, gap: 1 }}>
                                    <Button fullWidth variant="contained" sx={{ bgcolor: "red", color: "white", fontWeight: 600 }} onClick={clearAll}>
                                        CANCEL
                                    </Button>
                                    <Button fullWidth variant="contained" sx={{ bgcolor: "green", color: "white", fontWeight: 600 }} onClick={() => alert(`Placed ₹${stake} bet for odds ${bestSliptData.odds}`)}>
                                        PLACE BET
                                    </Button>
                                </Box>
                            </Box>
                        ) : (
                            <Box sx={{ textAlign: "center", py: 4 }}>
                                <Typography>No bets placed in the selected event</Typography>
                            </Box>
                        )}
                    </Paper>
                </Box>
            </Box>
        </Box>
    );
};

export default CommonMatch;